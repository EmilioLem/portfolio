% Main function for solving a linear regression problem using a genetic algorithm
function [best_individual, best_fitness] = las_vegas_genetic()

    % Problem parameters
    num_variables = 2; % Number of coefficients: a, b
    target_values = [2, 3]; % Target coefficients for the linear equation
    x = 0:0.1:10; % Input values for x
    y_real = target_values(1) * x + target_values(2); % Expected y values
